...
class ProductCart extends PureComponent {
  ...
  totalAmount = () =>
    this.props.selectedProducts
      ? this.props.selectedProducts.reduce((sum, current) => {
          return +(
            sum +
            current.productDetails.prices.find(
  ...
  totalTax = () =>
    this.state.totalAmount ? +(this.state.totalAmount * 0.21) : 0;
      ...
      <div className="cart">
        ...
        <div className="cart__checkout">
          ...
          <div className="cart__quantity">
            Quantity: <span>{this.state.totalQuantity}</span>
          </div>
          ...
          <button className="cart__order" onClick={this.onCheckoutClicked}>
            ORDER
          </button>
        </div>
        {this.state.showCheckout && (
          <Checkout closeCheckoutModal={this.closeCheckoutModal} />
        )}
      </div>
    );
  }
