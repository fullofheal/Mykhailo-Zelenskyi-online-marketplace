...
class DetailsInfo extends PureComponent {
  ...
  componentDidUpdate() {
    ...
    return selectedAttrNum === this.props.productDetails.attributes.length
      ? this.setState({
  ...
  addCartProduct = () => {
    return this.props.inStock && this.state.allAttributesSelected
  ...
  render() {
    const price = this.props.productDetails.prices
      ? this.props.productDetails.prices.find(
          (obj) => obj.currency.symbol === this.props.currencyType
        )
    ...
    return (
      <div className="details__info">
        ...
        <div className="details__price">
          ...
          <div className="details__price-value">
            {price.currency.symbol}
            {price.amount}
          </div>
        </div>
        <div className="details__cart">
          <button className="details__btn" onClick={this.addCartProduct}>
            ADD TO CART
          </button>
          {this.state.inStock ? (
            ""
          ) : (
            <div className="details__out-of-stock">
              This product is not available.
            </div>
          )}
          {this.state.showSelectAttributes && (
            <div className="details__out-of-stock">
              Please select available attributes.
            </div>
          )}
        </div>
        ...
        <div className="details__description">
          {this.props.productDetails.description && (
            <div>{parse(this.props.productDetails.description)}</div>
          )}
        </div>
      </div>
    );
