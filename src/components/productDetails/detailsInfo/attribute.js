...
class Attribute extends PureComponent {
  ...
  focusOnAttribute = (i, isColor) => {
    this.attributeRefs.forEach((attribute) =>
      attribute.classList.remove(
        isColor
          ? "details__attribute_color_selected"
          : "details__attribute_selected"
      )
    );
    this.attributeRefs[i].classList.add(
      isColor
        ? "details__attribute_color_selected"
        : "details__attribute_selected"
    );
  ...
  render() {
    const attributeWrapperClass = (attributeSet) => {
      return attributeSet.type === "swatch"
        ? "details__wrapper_color"
        : "details__wrapper";
    };
    ...
    const attributeClass = (attribute, attributeSet, isColor) => {
      if (isColor) {
        return this.props.selectedProduct
          ? this.props.selectedProduct.selectedAttributes.Color ===
            attribute.value
            ? "details__attribute_color details__attribute_color_selected"
            : "details__attribute_color"
          : "details__attribute_color";
      } else {
        if (this.props.selectedProduct) {
          const selectedAttribute =
            this.props.selectedProduct.selectedAttributes[attributeSet.id];
          return selectedAttribute === attribute.value
            ? "details__attribute details__attribute_selected"
            : "details__attribute";
        } else {
